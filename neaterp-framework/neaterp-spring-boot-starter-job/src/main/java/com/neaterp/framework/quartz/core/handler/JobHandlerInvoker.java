package com.neaterp.framework.quartz.core.handler;

import cn.hutool.core.date.LocalDateTimeUtil;
import cn.hutool.core.lang.Assert;
import cn.hutool.core.thread.ThreadUtil;
import com.neaterp.framework.quartz.core.enums.JobDataKeyEnum;
import com.neaterp.framework.quartz.core.service.JobLogFrameworkService;
 ...
 * @DisallowConcurrentExecution：同一个 JobDetail 的多个实例不能并发执行
 * @PersistJobDataAfterExecution：执行后更新 JobDataMap
 * 一起使用的效果如下：
 * 1、串行执行：避免并发修改 JobDataMap 导致数据竞争或不一致。
 * 2、状态延续：JobDataMap 的修改能传递到下一次执行，相当于 Job 有“记忆”。
 * @author 芋道源码
 */
